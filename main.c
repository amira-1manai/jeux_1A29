...
SDL_Event event;
SDL_Surface *back;
Personne p,p1;// declaration de 2 persos
...
initPerso(&p);
initPerso(&p1);//init de 2EME perso
    ...
    case SDL_KEYDOWN:
        switch (event.key.keysym.sym)
        {
            ...
            case SDLK_q:
                ...
                continuer=0;
                break;   
/////////////::input pour 2EME joueur 
                case SDLK_m:
                p1.direction=2;
                p1.vitesse=3;
                p1.acceleration+= 0.002;                
                break;
            case SDLK_k:
                p1.acceleration+=0.002;
                p1.vitesse=3;
...
if ((up1==1)&&(p1.position.y==200))  saut(&p1);

p1.position.y += p1.Yspeed;
p1.Yspeed += 10; //diminiuer le vitesse vertical

if (p1.position.y>=200)
{
    p1.position.y = 200;
    p1.Yspeed = 0;
}
...
//-----------affichage----------------//
    ...
    afficherPerso(p1,screen);//affichage 2EME perso
    SDL_Flip(screen);
